...
class RTOP_LA_paper(object):
    ...
    def __init__(self, nodes, num_costumers, num_terminals, LTL_orders, LTL_pattern, costumer_demands, number_of_trailers, trailer_capacity, number_experienced_drivers, c1, c2):
        ...
        self.J = range(self.num_terminals+1, len(self.nodes))
        self.V = [0]+[j for j in self.J]
        self.T = range(1, self.num_terminals+1)
    ...
    def setup(self):
        ...
        self.train_model.setObjective(
            gb.quicksum(self.c[t-1][i, j] * self.x[i, j, t, k] for k in self.K for t in self.T for i in self.V for j in self.J if(i < j)) +
            gb.quicksum(self.c_bar[0][t] * self.w[t] for t in self.T[1:])
        )
        ...
        for t in self.T[1:]:
            self.train_model.addConstr(
                gb.quicksum(self.y[t, k] for k in self.K) <= 3 * self.w[t] 
            )
        ...
        for j in self.J:
            for t in self.T:
                for k in self.K:
                    self.train_model.addConstr(
                        gb.quicksum(self.x[i, j, t, k] for i in self.V if (i < j)) +
                        gb.quicksum(self.x[j, i, t, k] for i in self.V if (i > j)) == 2 * self.z[t, k, j]
                    )
        # Constraint 7: \sum_{j \in J} x_{0jtk} = 2y_{tk} \hspace{1em} \forall k \in K \hspace{1em} \forall t \in T
        for k in self.K:
            for t in self.T:
                self.train_model.addConstr(
                    gb.quicksum(self.x[0, j, t, k] for j in self.J) == 2 * self.y[t, k]
                )
    ...
    def plot_solution(self):
        ...
        # blue: costumers
        colors = ['red'] * 1 + ['green'] * 1 + ['lime'] * (self.num_terminals - 1) + ['royalblue'] * (len(self.nodes) - (self.num_terminals + 1))        
        ...
        for t in self.T[1:]:
            if (self.w[t].x != 0): plt.plot((x_coords[1],x_coords[t]), (y_coords[1],y_coords[t]), 'g--')
                ...
                for j in self.J:
                    for i in self.V:
                        if (i < j):
                            ...
                            if(self.x[i,j,t,k].x == 2): plt.plot((x_coords[i_],x_coords[j_]), (y_coords[i_],y_coords[j_]), 'b-')
                            # light blue color for LTL orders
                            if(self.x[i,j,t,k].x == 1): plt.plot((x_coords[i_],x_coords[j_]), (y_coords[i_],y_coords[j_]), 'c-')
    ...
    def compute_cost_matrix(self):
        ...
        cost_matrix = [np.zeros((num_nodes, num_nodes)) for _ in range(self.num_terminals) for _ in range(self.num_terminals)]
        for t in range(self.num_terminals):
            for i in range(0, num_nodes):
                for j in range(0, num_nodes):
                    x1, y1 = self.nodes[i]
                    x2, y2 = self.nodes[j]
                    if i==0: x1, y1 = self.nodes[t+1]
                    if j==0: x2, y2 = self.nodes[t+1]
                    distance = np.sqrt((x2 - x1)**2 + (y2 - y1)**2)
                    # Highway case
                    if (i == 1 and j != 0 and j <= self.num_terminals) or (j == 1 and i != 0 and i < self.num_terminals+1):
                        cost_matrix[t][i][j] = round(distance * self.c2, 2)
                    # Standard road case
                    else:
                        cost_matrix[t][i][j] = round(distance * self.c1, 2)
        return cost_matrix
    ...
    def exctract_highway_costs(self, cost_matrix):
        ...
        highway_cost = [cost_matrix[t][1][0 : self.num_terminals + 1] for t in range(self.num_terminals)]
        return highway_cost
        # dictionary implementation: indices start from 2
        #highway_cost = road_cost_matrix[1][2 : self.num_terminals + 1]
        #indices = list(range(1, self.num_terminals+1))
